...
var (
	ErrFqdn      error = &DnsError{err: "domain must be fully qualified"}
	ErrId        error = &DnsError{err: "id mismatch"}
	ErrRdata     error = &DnsError{err: "bad rdata"}
	ErrBuf       error = &DnsError{err: "buffer size too small"}
	ErrShortRead error = &DnsError{err: "short read"}
	ErrConn      error = &DnsError{err: "conn holds both UDP and TCP connection"}
	ErrConnEmpty error = &DnsError{err: "conn has no connection"}
	ErrServ      error = &DnsError{err: "no servers could be reached"}
	ErrKey       error = &DnsError{err: "bad key"}
	ErrPrivKey   error = &DnsError{err: "bad private key"}
	ErrKeySize   error = &DnsError{err: "bad key size"}
	ErrKeyAlg    error = &DnsError{err: "bad key algorithm"}
	ErrAlg       error = &DnsError{err: "bad algorithm"}
	ErrTime      error = &DnsError{err: "bad time"}
	ErrNoSig     error = &DnsError{err: "no signature found"}
	ErrSig       error = &DnsError{err: "bad signature"}
	ErrSecret    error = &DnsError{err: "no secrets defined"}
	ErrSigGen    error = &DnsError{err: "bad signature generation"}
	ErrAuth      error = &DnsError{err: "bad authentication"}
	ErrSoa       error = &DnsError{err: "no SOA"}
	ErrRRset     error = &DnsError{err: "bad rrset"}
)
...
Loop:
	for {
		...
		case 0xC0:
			...
			if ptr++; ptr > 10 {
				return "", lenmsg, &DnsError{err: "too many compression pointers"}
			}
			off = (c^0xC0)<<8 | int(c1)
		default:
...
func packStructValue(val reflect.Value, msg []byte, off int, compression map[string]int, compress bool) (off1 int, err error) {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		default:
			return lenmsg, &DnsError{err: "bad kind packing"}
		case reflect.Slice:
			switch val.Type().Field(i).Tag {
			default:
				return lenmsg, &DnsError{err: "bad tag packing slice: " + val.Type().Field(i).Tag.Get("dns")}
			...
			case `dns:"txt"`:
				for j := 0; j < val.Field(i).Len(); j++ {
					...
					if len(element) > 255 || off+1+len(element) > lenmsg {
						return lenmsg, &OverflowError{err: "overflow packing txt"}
					}
			...
			case `dns:"opt"`: // edns
				for j := 0; j < val.Field(i).Len(); j++ {
					...
					if e != nil {
						return lenmsg, &OverflowError{err: "overflow packing opt"}
					}
					// Option code
			...
			case `dns:"a"`:
				...
				if off+net.IPv4len > lenmsg {
					return lenmsg, &OverflowError{err: "overflow packing a"}
				}
				switch fv.Len() {
				case net.IPv6len:
			...
			case `dns:"aaaa"`:
				...
				if fv.Len() > net.IPv6len || off+fv.Len() > lenmsg {
					return lenmsg, &OverflowError{err: "overflow packing aaaa"}
				}
			...
			case `dns:"wks"`:
				...
				for j := 0; j < val.Field(i).Len(); j++ {
					...
					if int(bitmapbyte) > lenmsg {
						return lenmsg, &OverflowError{err: "overflow packing wks"}
					}
			...
			case `dns:"nsec"`: // NSEC/NSEC3
				...
				length := uint16(0)
				if off+2 > lenmsg {
					return lenmsg, &OverflowError{err: "overflow packing nsecx"}
				}
				for j := 0; j < val.Field(i).Len(); j++ {
					...
					if lastwindow != window {
						...
						off += int(length) + 3
						if off > lenmsg {
							return lenmsg, &OverflowError{err: "overflow packing nsecx bitmap"}
						}
					}
		...
		case reflect.Uint8:
			if off+1 > lenmsg {
				return lenmsg, &OverflowError{err: "overflow packing uint8"}
			}
			msg[off] = byte(fv.Uint())
			off++
		case reflect.Uint16:
			if off+2 > lenmsg {
				return lenmsg, &OverflowError{err: "overflow packing uint16"}
			}
			i := fv.Uint()
			msg[off] = byte(i >> 8)
			msg[off+1] = byte(i)
		...
		case reflect.Uint32:
			if off+4 > lenmsg {
				return lenmsg, &OverflowError{err: "overflow packing uint32"}
			}
			i := fv.Uint()
			msg[off] = byte(i >> 24)
			msg[off+1] = byte(i >> 16)
		...
		case reflect.Uint64:
			...
			default:
				if off+8 > lenmsg {
					return lenmsg, &OverflowError{err: "overflow packing uint64"}
				}
				i := fv.Uint()
				msg[off] = byte(i >> 56)
				msg[off+1] = byte(i >> 48)
			...
			case `dns:"uint48"`:
				...
				if off+6 > lenmsg {
					return lenmsg, &OverflowError{err: "overflow packing uint64 as uint48"}
				}
				i := fv.Uint()
				msg[off] = byte(i >> 40)
				msg[off+1] = byte(i >> 32)
		...
		case reflect.String:
			...
			switch val.Type().Field(i).Tag {
			default:
				return lenmsg, &DnsError{err: "bad tag packing string: " + val.Type().Field(i).Tag.Get("dns")}
			case `dns:"base64"`:
				b64, err := packBase64([]byte(s))
				if err != nil {
					return lenmsg, &OverflowError{err: "overflow packing base64"}
				}
			...
			case `dns:"base32"`:
				...
				if err != nil {
					return lenmsg, &OverflowError{err: "overflow packing base32"}
				}
			...
			case `dns:"hex"`:
				...
				if e != nil {
					return lenmsg, &OverflowError{err: "overflow packing hex"}
				}
			...
			case "":
				...
				if len(s) > 255 || off+1+len(s) > lenmsg {
					return lenmsg, &OverflowError{err: "overflow packing string"}
				}
				msg[off] = byte(len(s))
				off++
				for i := 0; i < len(s); i++ {
...
func unpackStructValue(val reflect.Value, msg []byte, off int) (off1 int, err error) {
	...
	for i := 0; i < val.NumField(); i++ {
		switch fv := val.Field(i); fv.Kind() {
		default:
			return lenmsg, &DnsError{err: "bad kind unpacking"}
		case reflect.Slice:
			switch val.Type().Field(i).Tag {
			default:
				return lenmsg, &DnsError{err: "bad tag unpacking slice: " + val.Type().Field(i).Tag.Get("dns")}
			case `dns:"domain-name"`:
			...
			Txts:
				l := int(msg[off])
				if off+l+1 > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking txt"}
				}
			...
			Option:
				code := uint16(0)
				if off+2 > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking opt"}
				}
			...
			case `dns:"a"`:
				if off+net.IPv4len > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking a"}
				}
				fv.Set(reflect.ValueOf(net.IPv4(msg[off], msg[off+1], msg[off+2], msg[off+3])))
				off += net.IPv4len
			case `dns:"aaaa"`:
				if off+net.IPv6len > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking aaaa"}
				}
			...
			case `dns:"nsec"`: // NSEC/NSEC3
				...
				if off+2 > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking nsecx"}
				}
				nsec := make([]uint16, 0)
				length := 0
				window := 0
		...
		case reflect.Uint8:
			if off+1 > lenmsg {
				return lenmsg, &OverflowError{err: "overflow unpacking uint8"}
			}
		...
		case reflect.Uint16:
			var i uint16
			if off+2 > lenmsg {
				return lenmsg, &OverflowError{err: "overflow unpacking uint16"}
			}
			i, off = unpackUint16(msg, off)
			fv.SetUint(uint64(i))
		case reflect.Uint32:
			if off+4 > lenmsg {
				return lenmsg, &OverflowError{err: "overflow unpacking uint32"}
			}
		...
		case reflect.Uint64:
			...
			default:
				if off+8 > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking uint64"}
				}
			...
			case `dns:"uint48"`:
				...
				if off+6 > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking uint64 as uint48"}
				}
		...
		case reflect.String:
			...
			switch val.Type().Field(i).Tag {
			default:
				return lenmsg, &DnsError{err: "bad tag unpacking string: " + val.Type().Field(i).Tag.Get("dns")}
			case `dns:"hex"`:
				// Rest of the RR is hex encoded, network order an issue here?
				rdlength := int(val.FieldByName("Hdr").FieldByName("Rdlength").Uint())
				endrr := rdstart + rdlength
				if endrr > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking hex"}
				}
			...
			case `dns:"base64"`:
				...
				endrr := rdstart + rdlength
				if endrr > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking base64"}
				}
			...
			case `dns:"size-base32"`:
				...
				if off+size > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking base32"}
				}
			...
			case "":
				if off >= lenmsg || off+1+int(msg[off]) > lenmsg {
					return lenmsg, &OverflowError{err: "overflow unpacking string"}
				}
				n := int(msg[off])
				off++
				for i := 0; i < n; i++ {
...
func PackRR(rr RR, msg []byte, off int, compression map[string]int, compress bool) (off1 int, err error) {
	if rr == nil {
		return len(msg), &DnsError{err: "nil rr"}
	}
...
func UnpackRR(msg []byte, off int) (rr RR, off1 int, err error) {
	...
	if off != end {
		return &h, end, &DnsError{err: "bad rdlength"}
	}
	return rr, off, err
}
...
func (dns *Msg) Unpack(msg []byte) (err error) {
	...
	if off != len(msg) {
		return &OverflowError{err: "extra bytes in dns packet :" + strconv.Itoa(off) + "<" + strconv.Itoa(len(msg))}
	}
	return nil
}
